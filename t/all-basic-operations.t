...
sub Hashes {
  ...
  is_deeply scan_test('hscan', $key, 0, 'match', 'baz'), [qw( baz yikes )], 'hscan';
...
sub Keys {
  ...
  is_deeply $redis->keys("$key*"), [qw( redis2:test:Keys )], 'keys';
  is_deeply scan_test('scan', 0, 'match', $key, 'count', 100), [qw( redis2:test:Keys )], 'scan';
...
sub Sets {
  ...
  is_deeply scan_test('sscan', $key, 0, 'match', 'a'), [qw( a )], 'sscan';
...
sub SortedSets {
  ...
  is_deeply scan_test('zscan', $key, 0, 'match', 'two'), [qw( 5 two )], 'zscan';
...
sub scan_test {
  ...
  my ($res, %seen, $call);
  if ($method eq 'scan') {
    my $cursor = shift @args;
    $call = sub {
      $redis->scan($_[0] // $cursor, @args);
    };
  }
  else {
    my ($key, $cursor) = splice @args, 0, 2;
    $call = sub {
      $redis->$method($key, $_[0] // $cursor, @args);
    };
  }
  while ($guard--) {
    $res = $call->($res->[0]);
    $seen{$_} = 1 for @{$res->[1]};
    return [sort keys %seen] unless $res->[0];
  }
  return;
}
